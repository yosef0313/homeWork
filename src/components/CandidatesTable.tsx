...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case 'Interview': return { bg: '#fff3e0', color: '#ef6c00' };
    default: return { bg: '#e3f2fd', color: '#0288d1' };
  }
};
interface Candidate {
  id: number;
...
// UX: double-click copies the email to clipboard and shows visual feedback.
  ...
  return( <Container maxWidth="xl" sx={{ py: { xs: 2, md: 4 }, minHeight: '100dvh', bgcolor: '#f5f5f7' }}>
<Box sx={{ 
  mb: 4, 
  display: 'flex', 
...
<Box>
<Typography variant="h4" sx={{ fontWeight: 800, color: '#2c2c2c', letterSpacing: '-0.5px' }}>
Recruitment <span style={{ color: '#ea5b0c' }}>Portal</span>
</Typography>
...
Manage candidates efficiently
  ...
  {!isLoading && !error && (
          ...
          >
            <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#ea5b0c' }}>
    ...
    <TextField
      ...
      sx={{
        '& .MuiOutlinedInput-root': {
          borderRadius: 3,
          bgcolor: '#fafafa',
          '&.Mui-focused fieldset': { borderColor: '#ea5b0c' }
        }
      }}
    />
  </Grid>
    ...
    <TextField
      ...
      sx={{
        ...
        '& .MuiFormHelperText-root': {
            color: isPositionDisabled ? '#615b5b' : 'text.secondary',
            whiteSpace: 'nowrap' 
        }
      }}
    >
  ...
  <Grid size={{ xs: 12, sm: 6, md: 2 }}>
    <TextField
      ...
      sx={{
        ...
        '& .MuiFormHelperText-root': {
            color: isStatusDisabled ? '#615b5b' : 'text.secondary',
            whiteSpace: 'nowrap'
        }
      }}
    >
    ...
    {error ? (
      ...
      <Box sx={{ textAlign: 'center', py: 8 }}>
        <Typography variant="h6" color="text.secondary">No results found</Typography>
          ...
          {filteredCandidates.map((row) => {
            ...
            return (
              <Card key={row.id} variant="outlined" sx={{ borderRadius: 3, bgcolor: '#fafafa' }}>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    <Typography variant="h6" sx={{ fontWeight: 700, fontSize: '1rem' }}>{row.name}</Typography>
                    <Chip label={row.status} size="small" sx={{ bgcolor: colors.bg, color: colors.color, fontWeight: 700 }} />
                  </Box>
                  <Divider sx={{ my: 1.5 }} />
                  <Stack spacing={1.5}>
                        ...
                        <EmailIcon fontSize="small" color="action" />
                            ...
                            variant="body2" 
                              sx={{ 
                                ...
                                '&:hover': { color: '#ea5b0c' } // שינוי צבע במעבר
                              }}
                              onDoubleClick={() => handleCopyEmail(row.email)} // אירוע דאבל קליק
      ...
      <TableContainer>
        <Table sx={{ minWidth: 650 }}>
          <TableHead>
            <TableRow>
              <TableCell sx={{ color: '#9e9e9e', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>NAME</TableCell>
              <TableCell sx={{ color: '#9e9e9e', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>CONTACT</TableCell>
              <TableCell sx={{ color: '#9e9e9e', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>ROLE</TableCell>
              <TableCell sx={{ color: '#9e9e9e', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>EXPERIENCE</TableCell>
              <TableCell sx={{ color: '#9e9e9e', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>STATUS</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filteredCandidates.map((row) => (
              <TableRow
                key={row.id}
                sx={{
                  '&:hover': { bgcolor: 'rgba(234, 91, 12, 0.04)' },
                  transition: '0.2s'
                }}
              >
                <TableCell sx={{ fontWeight: 600, fontSize: '1rem', color: '#2c2c2c', borderBottom: '1px solid #f0f0f0' }}>
                  {row.name}
                </TableCell>
                <TableCell 
                    sx={{ 
                      ...
                      cursor: 'pointer', 
                      '&:hover': { color: '#ea5b0c', fontWeight: 500 }, 
                      transition: '0.2s'
                    }}
                ...
                <TableCell sx={{ borderBottom: '1px solid #f0f0f0' }}>
                  <Chip 
                    ...
                    size="small" 
                    sx={{ bgcolor: '#f5f5f7', fontWeight: 600, color: '#666' }} 
                  />
                </TableCell>
                <TableCell sx={{ borderBottom: '1px solid #f0f0f0' }}>
                  <Chip
                    ...
                    sx={{
                      ...
                      borderRadius: 2,
                      px: 1,
                      bgcolor: row.status === 'Hired' ? '#e8f5e9' :
                              row.status === 'Rejected' ? '#ffebee' :
                              row.status === 'Interview' ? '#fff3e0' : '#e3f2fd',
                      color: row.status === 'Hired' ? '#2e7d32' :
                             row.status === 'Rejected' ? '#d32f2f' :
                             row.status === 'Interview' ? '#ef6c00' : '#0288d1',
